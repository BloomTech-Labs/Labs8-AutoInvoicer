    ...
    return (
      <div className="container">
        ...
        {this.state.invoices && this.state.invoices.map(invoice => (
            <div key={invoice._id} className={invoice.balance_due === 0 ? "invoice paid"  : "invoice unpaid" }>
              <Link to={`/invoices/${invoice._id}`}>
                  <h4>Invoice #{invoice.invoice_number}</h4>
                  <p> Due Date: {invoice.due_date}</p>
                  <p> Company: {invoice.company_name}</p>
                  <p> {invoice.balance_due === 0 ? "Status: Payment Complete" : `Remaining Balance: ${accounting.formatMoney(invoice.balance_due)}`} </p>
                  <p className="late"> {invoice.due_date < Date.now() && invoice.balance_due  > 0 ? "Status: Overdue" : "" }</p> 
              </Link>
            </div>
          ))
        }
